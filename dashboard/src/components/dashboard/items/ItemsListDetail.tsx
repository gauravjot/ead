...
export default function ItemsListDetailPanel({id}:{id:number|string}){
  ...
  <>
      <div className="border-b">
            ...
            className={(activeTab === "list" ? "text-dodger-700 border-dodger-600" : "text-gray-600 border-transparent") + " border-b-2 p-2 active:bg-dodger-50"}>
						Item List
					</button>
          <button
            onClick={()=>{setActiveTab("edit")}}
            className={(activeTab === "edit" ? "text-dodger-700 border-dodger-600" : "text-gray-600 border-transparent") + " border-b-2 p-2 active:bg-dodger-50"}>
            Edit
          </button>
				</div>
			</div>
