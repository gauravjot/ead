...
import UserAdminister from "./user/Administer";
...
export default function UserDetailPanel({userID}: {userID: string}) {
	...
	return userQuery.isSuccess && user ? (
		<>
			<div className="border-b sticky top-0 z-[5] bg-white">
				<div className="flex gap-6 px-8 py-6">
					...
					<div>
						...
						<span className="text-gray-500 text-bb">ID: {user.id}</span>
			...
			{activeTab === "administer" && <UserAdminister admin={adminContext.admin} user={user} />}
			{activeTab === "notes" && <UserNotes admin={adminContext.admin} user={user} />}
		</>
	) : userQuery.isLoading ? (
