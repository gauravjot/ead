	...
	return (
		<div className="relative">
			...
			>
				{props.buttonIcon && (
					<span
						className={"ic-md " + props.buttonIcon + (props.iconOnly ? " ic-gray-50" : " ic-white")}
					></span>
				)}
				{props.buttonText && <span className="pl-2 pr-4">{props.buttonText}</span>}
				{props.showExpandIcon && <span className="ic ic-white ic-down-arrow"></span>}
			</button>
			<div
				ref={dropDownMenuRef}
				aria-hidden={!dropDownActive}
				className="aria-hidable absolute z-50 bg-white top-8 right-0 min-w-[6rem] flex flex-col rounded-md shadow-md px-1 py-0.5 border border-gray-200"
			>
				{props.items.map((item) => {
					return (
						<button
							...
							className="disabled:opacity-50 disabled:hover:outline-none text-left w-max min-w-full py-1 pl-3 pr-4 my-1 flex place-items-center gap-2.5 hover:bg-gray-50 rounded focus:outline hover:outline hover:outline-1 focus:outline-dodger-600 hover:outline-gray-200 focus:outline-2"
						>
							{item.icon && <span className={"ic ic-gray-75 " + item.icon}></span>}
