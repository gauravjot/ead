...
export default function ServerSetup({setAdmin} : {setAdmin: Dispatch<SetStateAction<AdminType | null>>}) {
  ...
  return <div className="bg-white rounded-md px-12 py-24 shadow-lg grid grid-cols-2 gap-12">
    ...
    <fieldset disabled={mutation.isLoading || mutation.isSuccess}>
      <label className="block text-gray-600 text-sm py-1.5 mt-4" htmlFor="username1">Username</label>
      <input className="block w-full border px-3 py-1.5 rounded focus-visible:outline-3 focus-visible:outline-gray-200 focus-visible:outline focus-visible:border-gray-300 disabled:bg-blue-50 disabled:text-gray-500" type="text" id="username1" name="username" value="root" disabled={true}/>
      <label className="block text-gray-600 text-sm py-1.5 mt-2" htmlFor="password1">Password</label> 
      <input className={"block w-full border px-3 py-1.5 rounded focus-visible:outline-3 focus-visible:outline-gray-200 focus-visible:outline focus-visible:border-gray-300" + (error.length > 0 && password.length < 1 ? " border-red-500/70": "")} type="password" id="password1" name="password" value={password} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>{setPassword(e.target.value)}}/>
       <Button 
